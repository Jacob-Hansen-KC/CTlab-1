...
# define all variables in c data types
    ...
    cdef np.ndarray[np.float64_t, ndim=1] bdry1 = np.array([500.0, 200.0, 300.0, 700.0], dtype=np.float64)
    #S[24, 24] = 100.0
        ...
        for i in range(1, s+1):
            for j in range(1, s+1):
                T1[i, j] = (
                    T2[i, j]
                    + alpha * dt * (
                        (T2[i+1, j] - 2.0*T2[i, j] + T2[i-1, j]) * dx2
                        + (T2[i, j+1] - 2.0*T2[i, j] + T2[i, j-1]) * dy2
                    )
                    + S[i, j] * dt
                )
        k += 1
